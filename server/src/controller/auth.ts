...
const login=async(req:Request,res:Response)=>{
    try {
        ...
        if (!User || !password){
            return  res.send({message:"usuario o contraseña incorrecto", valid:false})
         } 
    ...
    if(!token){
        return res.send({message:"no se pudo validar al usuario",valid:false})
    }
...
const register=async(req:Request,res:Response)=>{
    try {
        ...
        if(exist){
            return res.send({message:"El usuario ya existe en la base de datos",valid:false})
        }
        
        if(password.length<=6){
            return res.send({message:"El password debe ser mayo a 6 caracteres",valid:false})
        }
